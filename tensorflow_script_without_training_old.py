...
def start_tensorflow_without_training(paths_for_Blender, networkPrefix, test_model_name):
	...
	np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"-"+test_model_name+'test_model_rest_pose.out', test_model_mesh_rest_pose, fmt="%.6f")
	...
	#test_model_mesh_poses = test_model.get_approximated_v()
	#np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"-"+test_model_name+'test_model_approxFinal.obj', test_model_mesh_poses, fmt="%.6f")
	...
	for f in range(0, test_model.num_of_frames):
		np.savetxt(os.path.dirname(__file__)+'\\'+networkPrefix+"-"+test_model_name+"_approx"+str(f)+".obj", Approximated[:,:,f], fmt="%.6f")
'''
